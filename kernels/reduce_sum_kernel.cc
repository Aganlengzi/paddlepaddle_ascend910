...
void SumRawKernel(const Context& dev_ctx, const phi::DenseTensor& x,
  ...
  if (reduce_all) {
    ...
    const auto& runner = NpuOpRunner("ReduceSumD", {cast_x}, {cast_out},
...
template <typename T, typename Context>
void SumKernel(const Context& dev_ctx, const phi::DenseTensor& x,
...
PD_REGISTER_PLUGIN_KERNEL(sum_raw, Ascend910, ALL_LAYOUT,
                          custom_kernel::SumRawKernel, int32_t, int64_t,
                          phi::dtype::float16, float) {}

// PD_REGISTER_PLUGIN_KERNEL(sum,
//                           Ascend910,
//                           ALL_LAYOUT,
//                           custom_kernel::SumKernel, int32_t, int64_t,
//                           phi::dtype::float16, float) {}
