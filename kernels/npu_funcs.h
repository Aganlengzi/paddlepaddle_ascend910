...
inline void TensorFromVector(const phi::CustomContext& ctx, const std::vector<T>& src,
  ...
  if (dst_place.GetType() == phi::AllocationType::CUSTOM) {
    AsyncMemCpyH2D(nullptr, static_cast<C_Stream>(dev_ctx.stream()), dst_ptr, src_ptr, size);
  }
  else {
...
inline void NpuBroadcast(const Context& dev_ctx, const phi::DenseTensor* src,
