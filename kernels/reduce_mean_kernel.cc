...
void MeanRawKernel(const Context& dev_ctx, const phi::DenseTensor& x,
                     const std::vector<int64_t>& axes, bool keep_dim,
                     bool reduce_all, phi::DenseTensor* out) {
  auto dims = axes;
  ...
  const auto& runner = NpuOpRunner("ReduceMeanD", {x}, {*out},
...
template <typename T, typename Context>
void MeanKernel(const Context& dev_ctx, const phi::DenseTensor& x,
...
PD_REGISTER_PLUGIN_KERNEL(mean_raw, Ascend910, ALL_LAYOUT,
                          custom_kernel::MeanRawKernel, float) {}
...
PD_REGISTER_PLUGIN_KERNEL(mean_grad,
                          Ascend910,
                          ALL_LAYOUT,
                          custom_kernel::MeanGradKernel, float) {}
