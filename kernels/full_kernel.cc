  ...
  if (dtype != phi::DenseTensorMeta::DataType::BOOL) {
    ...
    runner.SetType("Fill")
      ...
      auto op_func = [&shape_vec, &value](
          const std::vector<phi::DenseTensor> &inputs, const std::vector<phi::DenseTensor>
          &outputs,
          const NPUAttributeMap &attrs,
          const Context &dev_ctx) {
        phi::DenseTensor tensor_value;
        tensor_value.Resize(phi::make_ddim({1}));
        FillNpuTensorWithConstant<uint8_t>(&tensor_value, dev_ctx, static_cast<uint8_t>(value));
      ...
      NpuOpRunner::TypeAdapter({}, {*out}, {}, dev_ctx, op_func, {},
