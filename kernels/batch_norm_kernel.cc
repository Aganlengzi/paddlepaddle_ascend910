...
void BatchNormKernel(const Context& dev_ctx,
    ...
    phi::DataLayout data_layout = paddle::framework::StringToDataLayout(data_layout_str);
    ...
    if (d_x) {
      ...
      if (use_global_stats) {
        ...
        const auto &runner_infer =
            NpuOpRunner("BNInferGrad", {dy_tensor, scale, *running_variance},
...
PD_REGISTER_PLUGIN_KERNEL(batch_norm, Ascend910, ALL_LAYOUT,
                          custom_kernel::BatchNormKernel, phi::dtype::float16,
                          float, double) {}
