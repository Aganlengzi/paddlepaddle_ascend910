...
static void ReduceDims(const Context& dev_ctx,
                       ...
                       const std::vector<int64_t>& brd_dims, const phi::DenseTensor& in,
                       phi::DenseTensor* out) {
...
void MatmulKernel(const Context& dev_ctx,
    ...
    MatMulND<T>(dev_ctx, stream, x_temp_brd, y_temp_brd, out, transpose_x, transpose_y);
  }
...
void MatmulGradKernel(const Context& dev_ctx,
    ...
    if (out_ndim == 2) {
      ...
      if (dy) {
        ...
        if (transpose_y) {
          ...
          MatMul2D<T>(dev_ctx, stream, x_temp, dout_temp, dy, !transpose_x, false);
        }
        dy->Resize(y.dims());
      }
      return;
    ...
    if (dx) {
      if (x_dims == x_broadcast_dims) {
        if (transpose_x) {
          MatMulND<T>(dev_ctx, stream, y_temp_brd, dout_temp, dx, transpose_y, true);
        } else {
        ...
        if (transpose_x) {
          ...
          MatMulND<T>(dev_ctx, stream, dout_temp, y_temp_brd, &dx_temp, false,
                      !transpose_y);
        }
    ...
    if (dy) {
      if (y_dims == y_broadcast_dims) {
        if (transpose_y) {
          MatMulND<T>(dev_ctx, stream, dout_temp, x_temp_brd, dy, true, transpose_x);
        } else {
        ...
        if (transpose_y) {
          ...
          MatMulND<T>(dev_ctx, stream, x_temp_brd, dout_temp, &dy_temp, !transpose_x,
                      false);
        }
...
PD_REGISTER_PLUGIN_KERNEL(matmul,
                          Ascend910,
                          ALL_LAYOUT,
                          custom_kernel::MatmulKernel, float, phi::dtype::float16) {}

PD_REGISTER_PLUGIN_KERNEL(matmul_grad,
                          Ascend910,
                          ALL_LAYOUT,
                          custom_kernel::MatmulGradKernel, float, phi::dtype::float16) {}
